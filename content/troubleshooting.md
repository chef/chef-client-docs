...
2. Make sure to use the same `node_name` as the initial Chef Infra Client run.
    ...
    This can happen for several reasons. For example, if the
...
Solve this by committing the cookbook changes. For example, the following command would commit all new changes with a message.
